...
\expl{Places subsequent text and lines at graph position (\emph{x},
  ...
  directions. \cref{at} may also be used to mark a location for later
  reference, as per the final two syntax forms.}
...
\expl{Defines a new panel with given ID to have its top left corner on
  ...
  \cref{xlim} and \cref{ylim} affect the axes in the
...
\expl{Places the given text \emph{string} at the position (\emph{dx},
  ...
  (up to the next space, which is swallowed). Use tilde as a protected
  space inside subscripts and superscripts.
  Slashes and asterisks make enclosed words
  appear in /italics/ and *bold*. Unicode is supported. Backslash
  protects interpretation of special characters, but backslash + comma
  and backslash + exclamation point insert and remove space as in
  TeX equations. The string
