...
\expl{Places subsequent text and lines at graph
    ...
    bounding box of the last drawn object, or one of cleft, cright,
    ccenter to place relative to the cumulative bounding box. (See
    \cref{cumul}.) Likewise, \emph{y} may be one of top, bottom,
    middle, ctop, cbottom, cmiddle.}
...
\cmd{cumul}{}
\expl{Restarts accumulation of bounding boxes for \cref{at}\n}
...
\noindent where \emph{source} is a file with commands and
...
\noindent In that case, graphics are rendered in a window. Keys ``+''
