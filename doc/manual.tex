...
\expl{Places subsequent text and lines at graph
    ...
    reverts to  placement relative to the top left of the
    current panel
...
\cmd{hairline}{\emph{width}}
\expl{Specifies a width for lines plotted with zero nominal width, in
  points. Default: 0.25 pt. For raster output, it may be useful to
set ``hairline 0'', which produces lines of precisely one pixel
wide. This is not recommended for pdf or svg output, since the
resulting file would become device dependent.}
...
\expl{Renders markers set by \cmd{marker} at the given data
  coordinates.}
...
3~pts.
  ...
  subcommands may be given on one line in any order. A \cmdref{pen}
