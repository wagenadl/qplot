...
void prerender(Program &prog, Figure &fig) {
  ...
  QPainter &ptr(fig.painter());
  ptr.begin(&img);
  //ptr.scale(iu2pt(), iu2pt());
  fig.reset();
...
bool renderSVG(Program &prog, Figure &fig, QString ofn) {
  ...
  if (!ptr.begin(&img))
    return false;
  ptr.scale(90./72*iu2pt(), 90./72*iu2pt());
  fig.setDashScale(90./72*iu2pt());
  ptr.translate(fig.extent().left(),
			  fig.extent().top());
  prog.render(fig);
  ptr.end();
  return true;
...
bool renderPDF(Program &prog, Figure &fig, QString ofn) {
  ...
  ptr.setRenderHint(QPainter::LosslessImageRendering);
#else
  #warning Images will be rendered to pdf in jpeg format - Use Qt >= 5.13 to avoid this issue
#endif

  double dpix = img.logicalDpiX();
  double dpiy = img.logicalDpiY();

  ptr.translate(-10*dpix/72., -10*dpiy/72.);
  // I don't know why this translation is needed.

  ptr.scale(iu2pt()*dpix/72.0,
		      iu2pt()*dpix/72.0);
...
bool renderPS(Program &prog, Figure &fig, QString ofn, QString ttl="") {
  ...
  if (!ptr.begin(&img))
    return false;
  ptr.translate((p.width()-imsize.width())/2,
			  (p.height()-imsize.height())/2);

  /* Draw some crop marks */
  ptr.save();
  { QPen p; p.setWidth(.5); ptr.setPen(p); }
  const int MINX = 5;
  const int MAXX = 20;
  // tl
  ptr.drawLine(-MAXX,0,-MINX,0);
  ptr.drawLine(0,-MAXX,0,-MINX);
  // bl
  ptr.drawLine(-MAXX,imsize.height(),-MINX,imsize.height());
  ptr.drawLine(0,imsize.height()+MINX,0,imsize.height()+MAXX);
  // tr
  ptr.drawLine(imsize.width()+MINX,0,imsize.width()+MAXX,0);
  ptr.drawLine(imsize.width(),-MINX,imsize.width(),-MAXX);
  // tr
  ptr.drawLine(imsize.width()+MINX,imsize.height(),
			 imsize.width()+MAXX,imsize.height());
  ptr.drawLine(imsize.width(),imsize.height()+MINX,
			 imsize.width(),imsize.height()+MAXX);
  // render title
  ptr.setFont(QFont("Helvetica", 10));
  ptr.drawText(10, imsize.height()+18, ttl);
  ptr.restore();
